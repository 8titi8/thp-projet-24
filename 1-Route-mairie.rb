...
puts "Merci de ne pas quitter, c'est un peu long à télécharger ! :)"
	...
	def get_the_email_of_a_townhall_from_its_webpage(array_of_link)
		...
		array_of_link.each do |link|
			...
			array_of_emails << {"email" => page.css("div main section[2] div table tbody tr[4] td[2]").text }
			end
	...
	def get_all_the_urls_of_val_doise_townhalls
		...
		array_of_name = []
		@data.each do |name_town|
			array_of_name << name_town.text
			end
			 		
		# creation tableau des liens html de chaque ville
		array_of_links = []
		@data.each do |link_townhall|
			array_of_links << link_townhall['href']
		end
	...
	list_emails = get_the_email_of_a_townhall_from_its_webpage(get_all_the_urls_of_val_doise_townhalls)
